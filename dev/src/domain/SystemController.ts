...
import { ImmediatePurchasePolicyComponent } from "./marketplace/DiscountAndPurchasePolicies/Components/ImmediatePurchasePolicyComponent";
...
export class SystemController {
  ...
  editCart(
    ...
    const authCallback = (id: string) => {
      ...
      if (checkRes(productRes)) {
        return this.scController.updateProductQuantity(
          id,
          productRes.data,
          quantity
        );
      } else return new Result(false, undefined, "product not found");
    };
    return this.authenticateMarketVisitor(sessionId, authCallback);
  }
    ...
    const authCallback = (founderId: string): Result<void> => {
      ...
      if (checkRes(result)) {
        ...
        return new Result(
          false,
          undefined,
          "you have to be a member to open a shop."
        );
      }
      //found shop
  ...
  addProduct(
    ...
    const authCallback = (id: string) => {
      if (
        this.uController.checkPermission(id, p.shopId, Permissions.AddProduct)
          .data
      ) {
  ...
  updateProductQuantity(
    ...
    const authCallback = (id: string): Result<void> => {
      if (
        this.uController.checkPermission(id, shop, Permissions.ModifyProduct)
          .data
      ) {
  ...
  deleteProduct(sessId: string, shop: number, product: number): Result<void> {
    const authCallback = (id: string): Result<void> => {
      if (
        this.uController.checkPermission(id, shop, Permissions.RemoveProduct)
          .data
      ) {
  ...
  deactivateShop(sessId: string, shop: number): Result<void> {
    return this.authenticateMarketVisitor(sessId, (id: string) => {
      if (
        this.uController.checkPermission(id, shop, Permissions.CloseShop).data
      ) {
  ...
  appointShopManager(sessionId: string, r: NewRoleData): Result<void> {
    const authCallback = (appointerId: string) => {
      if (
        ...
        ); //todo: adding an assigner to the method?
        if (result.ok) {
  ...
  removeShopManagerPermission(
    ...
    const authCallback = (id: string) => {
      if (
        this.uController.checkPermission(id, shop, Permissions.RemovePermission)
          .data
      ) {
  ...
  reactivateShop(sessId: string, shop: number): Result<void> {
    return this.authenticateMarketVisitor(sessId, (id: string) => {
      if (
        this.uController.checkPermission(id, shop, Permissions.ReopenShop).data
      ) {
    ...
    const callback = (id: string) => {
      if (
        !this.uController.checkPermission(
          id,
          shop,
          Permissions.RequestPersonnelInfo
        ).data
      ) {
        return new Result(false, undefined, "no permission");
      }
    ...
    const callback = (id: string) => {
      ...
      if (
        !this.uController.checkPermission(
          id,
          shop,
          Permissions.GetPurchaseHistory
        ).data
      ) {
        return new Result(false, undefined, "no permission");
      }
      ...
      return new Result(
        true,
        orders,
        orders.length !== 0 ? undefined : "no SimpleShop order were found"
      );
    };
  ...
  registerAsAdmin(
    sessionID: string,
    registrationData: RegisterMemberData,
  ...
  editConnectionWithExternalService(
    ...
    return this.authenticateMarketVisitor(admin, (id: string) => {
      ...
      else this.pController.paymentService.editServiceSettings(settings);
