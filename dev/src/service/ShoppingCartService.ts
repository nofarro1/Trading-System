...
export class ShoppingCartService {
    ...
    addToCart(sessionID: string, shopId: number, productID: number, productQuantity: number): Promise<Result<void>> {
        return new Promise<Result<void>>(async (resolve, reject) => {
            let result: Result<void> = await this.systemController.addToCart(sessionID, shopId, productID, productQuantity);
    ...
    checkShoppingCart(sessionID: string): Promise<Result<void | SimpleShoppingCart>> {
        return new Promise<Result<void | SimpleShoppingCart>>(async (resolve, reject) => {
            let result: Result<void | SimpleShoppingCart> = await this.systemController.getCart(sessionID);
            result.ok ? resolve(result) : reject(result.message);
        });
    }
